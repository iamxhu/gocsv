...
func Test_readToWithCustomizeTag(t *testing.T) {
	//add this line to setting customize Tag name
	os.Setenv("tag","json")
	blah := 0
	sptr := "*string"
	sptr2 := ""
...
e,3,b,,,`)
	...
	expected := SampleTagJson{Foo: "f", Bar: 1, Baz: "baz", Blah: &blah, SPtr: &sptr, Omit: &sptr}
	if !reflect.DeepEqual(expected, samples[0]) {
		t.Fatalf("expected first sample %v, got %v", expected, samples[0])
	}

	expected = SampleTagJson{Foo: "e", Bar: 3, Baz: "b", Blah: &blah, SPtr: &sptr2}
	if !reflect.DeepEqual(expected, samples[1]) {
		t.Fatalf("expected second sample %v, got %v", expected, samples[1])
	}
